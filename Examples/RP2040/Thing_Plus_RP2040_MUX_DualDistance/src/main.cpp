...
void setup()
{
  ...
  for (int x = 0; x < NUMBER_OF_SENSORS; x++)
    distanceSensor[x] = new SFEVL53L1X(Wire1);

  if (myMux.begin(QWIIC_MUX_DEFAULT_ADDRESS, Wire1) == false)
  {
  ...
  {
    myMux.setPort(x);
    if (distanceSensor[x]->begin(Wire1) != 0) //Begin returns 0 on a good init
    {
      Serial.print("Sensor ");
      Serial.print(x);
