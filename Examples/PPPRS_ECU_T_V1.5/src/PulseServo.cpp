/*
 PulseServo.cpp - Interrupt driven Servo library for Arduino using 16 bit timers- Version 2
...
/* 
 ...
 A servo is activated by creating an instance of the PulseServo class passing the desired pin to the attach() method.
 ...
 PulseServo - Class for manipulating servo motors connected to Arduino pins.
...
uint8_t PulseServo::attach(int pin)
{
  return this->attach(pin, MIN_PULSE_WIDTH, MAX_PULSE_WIDTH);
}

uint8_t PulseServo::attach(int pin, int min, int max)
{
...
void PulseServo::write(int value)
{  
...
int PulseServo::writeMicroseconds(float value)
{
  ...
  {  
    ...
    value = (( clockCyclesPerMicrosecond()* value) / 8.0)  // convert to ticks after compensating for interrupt overhead - 12 Aug 2009
	int intvalue = int(value);
	
	//
	timer16_Sequence_t timer = SERVO_INDEX_TO_TIMER(channel);
	#ifndef WIRING
	#if defined(_useTimer1)
	OCR1A = TCNT1 + 8;  // REFRESH_INTERVAL has elapsed and we need to force an interrupt in a couple cycles
	#endif
	#if defined(_useTimer3)
	OCR3A = TCNT3 + 8;  // REFRESH_INTERVAL has elapsed and we need to force an interrupt in a couple cycles
	#endif
	#if defined(_useTimer4)
	OCR4A = TCNT4 + 8;  // REFRESH_INTERVAL has elapsed and we need to force an interrupt in a couple cycles
	#endif
	#if defined(_useTimer5)
	OCR5A = TCNT5 + 8;  // REFRESH_INTERVAL has elapsed and we need to force an interrupt in a couple cycles	
	#endif
	#elif defined WIRING
	#if defined(_useTimer1)
	OCR1A = TCNT1 + 8;  // REFRESH_INTERVAL has elapsed and we need to force an interrupt in a couple cycles
    ...
    cli();
    servos[channel].ticks = intvalue;  
    SREG = oldSREG;   
	return intvalue;
  } 
}

int PulseServo::read() // return the value as degrees
{
  return  map( this->readMicroseconds()+1, SERVO_MIN(), SERVO_MAX(), 0, 180);     
}

int PulseServo::readMicroseconds()
{
...
volatile boolean resetcounter = false;
...
uint8_t PulseServo::attach(int pin, int minimum, int maximum)
{
...
int PulseServo::writeMicroseconds(float value)
{
	value = (( clockCyclesPerMicrosecond()* value) / 8.0f);
	if (value < min_ticks) {
		value = min_ticks;
	...
	//
	resetcounter = true;
	PDB0_IDLY = PDB0_CNT + usToTicks(5);
	PDB0_SC = PDB_CONFIG | PDB_SC_LDOK;
	//
	
	servo_ticks[servoIndex] = intvalue;
	return intvalue;
}
...
extern "C" void pdb_isr(void)
{
	...
	if (resetcounter){wait_ticks = usToTicks(5);}
	else{if(wait_ticks < usToTicks(100)){ wait_ticks = usToTicks(100);}
	...
	if (resetcounter||(tick_accum >= usToTicks(REFRESH_INTERVAL))) {
		tick_accum = 0;
		channel = 0;
		resetcounter = false;
	}
...
int PulseServo::writeMicroseconds(float value)
{
	value = (( clockCyclesPerMicrosecond()* value) / 8.0)
	if (value < min_ticks) {
		value = min_ticks;
	} else if (value > max_ticks) {
		value = max_ticks;
