...
unsigned long startEdgeLearningMillis;
unsigned long currentMillis;
const unsigned long period = 10*ONE_SECOND; // 60 seconds in ms
bool edgeLearningDone = false;
long shortestDistance = 45000; // Larger than sensor max range of 4000mm / 4m
...
bool isEdgeLearning(){
    ...
    int timeRemaining = (period - (currentMillis - startEdgeLearningMillis)) / ONE_SECOND;
    ...
    if(timeRemaining <= 0){
      ...
      Serial.println("mm");
      edgeLearningDone = true;

      String distStr = String(shortestDistance); 
...
void setupEdgeLearning(){
   startEdgeLearningMillis = millis();
}
  ...
  for(uint8_t i = 0; i < MATRIX_ROWS; i++){
    if(i == ROW_EDGE_DETECTION ){
      for(uint8_t j = 0; j < MATRIX_COLS; j++){
        ...
        if(val < shortestDistance && val > 20){
